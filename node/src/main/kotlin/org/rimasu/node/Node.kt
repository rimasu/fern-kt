...
class NullNode : Node() {
    override fun <T> incompatibleValue(path: Path) = Result.err(IncompatibleValue(path))
}
...
class StructNode(values: Map<String, Node>) : Node() {
    ...
    operator fun get(label: String) : Result<Node, NodeError> {
        ...
        return if (value != null) {
            ...
            Result.err(UndefinedValue(path + LabelStep(label)))
        }
    }
...
class ListNode(nodes: List<Node>) : Iterable<Node>, Node() {
    ...
    operator fun get(index: Int) : Result<Node, NodeError> {
        return when {
            index < 1 -> Result.err(UndefinedValue(path + IndexStep(index)))
            index > values.size -> Result.err(UndefinedValue(path + IndexStep(index)))
